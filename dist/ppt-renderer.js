...
function semanticSlides(
  ...
  const renderItems = (items                                        , notes         )         => items.map((item) => item.source_refs.map((ref) => {
    ...
    if (semantic.kind === "exercise") {
      const encoded = JSON.parse(semantic.body)            ;
    ...
    return marked.lexer(semantic.body).map((token) => block(token         , media)).join("");
  }).join("") + (item.mode === "authored" ? marked.lexer(item.text).map((token) => block(token         , media)).join("") : "")).join("");
  return plan.slides
...
function deckHtml(
  ...
  const slides          = planOwned ? contentSlides : [
    {
...
export async function renderPptFromBrief(root        , briefId        , outputDirectory        , plan           , packet               )                  {
