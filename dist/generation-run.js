...
export class GenerationAttentionRequired extends Error {
  ...
  constructor(attemptRef        , actionCode = "inspect-publication-attempt") {
...
async function cached(root        , runId        , inputHash        , version        )                                           {
  try {
    const verified = await readContentCache                     (root, "generation-run", inputHash, version);
    if (!verified || verified.run_id !== runId || verified.state !== "complete") return undefined;
    const value = JSON.parse(await readFile(path.join(root, ".k-teach", "runs", `${runId}.json`), "utf8"))                       ;
...
//# sourceURL=k-teach/src/generation-run.ts