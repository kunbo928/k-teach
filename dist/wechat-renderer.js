...
function selectBlocks(markdown        , brief                  )          {
  ...
  const flush = () => {
    ...
    const publicBody = body.filter((token) => !(
      ...
      /^\{\{exercise:[A-Za-z0-9][A-Za-z0-9_-]*\}\}\s*$/.test((token                    ).text)
    ));
    if (publicBody.length > 0) sanitized.push(heading, ...publicBody);
    section = [];
  };
  for (const token of selected) {
    if (token.type === "heading" && (token                  ).depth === 2) flush();
    section.push(token);
  }
  flush();
  return sanitized;
...
export async function renderWechatProposals(root        , briefId        , outputDirectory        )                  {
