...
import { validateSemanticPlan } from "./semantic-plan.js";
  ...
  return tokens
    .map((token) => {
      if (token.type === "text") {
        ...
        return nested ? renderInline(nested, context) : leaf((token               ).text);
      }
      ...
      if (token.type === "del") {
        return `<span style="text-decoration:line-through;color:${
          COLORS.muted
        };">${renderInline((token              ).tokens, context)}</span>`;
      }
...
function selectPlanBlocks(plan             , packet               )          {
...
function wrapPreview(title        , article        )         {
  return `<!doctype html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8">
...
function copyArticleRuntime(articleExpression        )         {
...
function teachingThemeForChannel(id                )                {
...
function wrapProposalPreview(title        , articles                                , selected                )         {
...
async function prepareCover(
  ...
  for (const directory of planDirectories.filter((entry) => entry.isDirectory())) {
    ...
    try {
      ...
      if (errors.length === 0) matches.push(record                           );
    } catch {
...
export async function renderWechat(
  ...
  if (options.proposals) {
    for (const [id, candidate] of Object.entries(proposalArticles                                  )) {
...
export async function renderWechatProposals(root        , briefId        , outputDirectory        , plan             , packet               )                  {
  return renderWechat(root, briefId, outputDirectory, { proposals: true, plan, packet });
}


//# sourceURL=k-teach/src/wechat-renderer.ts