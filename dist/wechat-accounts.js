...
export async function addWechatAccount(account               , file = wechatAccountsPath())                {
...
export async function requireWechatAccount(alias        , file = wechatAccountsPath())                         {
  const registry = await readWechatAccounts(file);
...
export async function markWechatAccountSuccessful(alias        , file = wechatAccountsPath())                {
  const registry = await readWechatAccounts(file);
...
export function maskedAppId(appId        )         {
  return `${"•".repeat(Math.min(6, Math.max(2, appId.length - 6)))}${appId.slice(-6)}`;
}


//# sourceURL=k-teach/src/wechat-accounts.ts