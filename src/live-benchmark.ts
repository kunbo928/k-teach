...
export interface LiveScenario { id: string; route: "learn" | "ppt" | "wechat"; phase: "cold" | "warm" | "delta" }
export interface LiveBenchmarkConfig {
  ...
  generated_integration_hash: string;
  repetitions: 3;
  reasoning_effort: string;
  sampling: Record<string, number | string | boolean | null>;
  prompt_hashes: { system: string; developer: string; user: string };
  ...
  packages: {
    baseline: { root: string; name: string; version: string; expected_root_hash: string };
    candidate: { root: string; name: string; version: string; expected_root_hash: string };
  };
...
export interface LiveSample {
  ...
  repetition: number;
  attempt: number;
  usage: { uncached_input: number; cached_input: number; output: number; control: number; semantic: number };
  requests: number;
  retries: number;
  tool_payload_tokens: { arguments: number; results: number };
  optional_reference_loads: string[];
  cli_invocations: string[];
  artifact_hash: string;
  manifest_hash: string;
  generation_state: "complete";
  plan_review_items: number;
  quality_passed: boolean;
  quality_assertions: string[];
  model_authored_layout_code_tokens: number;
  elapsed_ms: number;
  failure?: { kind: "environmental" | "content"; code: string; model_executed: boolean };
}

const digest = /^[a-f0-9]{64}$/;
const sampleKey = (sample: Pick<LiveSample, "variant" | "scenario" | "repetition">) => `${sample.variant}/${sample.scenario}/${sample.repetition}`;

export function validateLiveConfig(value: unknown): LiveBenchmarkConfig {
  if (!value || typeof value !== "object") throw new Error("Missing live benchmark configuration.");
  const config = value as Partial<LiveBenchmarkConfig>;
  for (const field of ["provider", "model", "model_snapshot", "agent_version", "command_version", "reasoning_effort"] as const) {
  ...
  return config as LiveBenchmarkConfig;
}

export function interleavedSchedule(scenarios: LiveScenario[], repetitions = 3) {
  return Array.from({ length: repetitions }, (_, index) => scenarios.flatMap((scenario) => [
    { variant: "baseline" as const, scenario, repetition: index + 1 },
    { variant: "candidate" as const, scenario, repetition: index + 1 },
  ])).flat();
}
...
function successfulSamples(scenarios: LiveScenario[], samples: LiveSample[]): LiveSample[] {
...
export function evaluateLiveSamples(scenarios: LiveScenario[], samples: LiveSample[]) {
  const selected = successfulSamples(scenarios, samples);
  for (const sample of selected) {
    for (const field of ["uncached_input", "cached_input", "output", "control", "semantic"] as const) if (!Number.isFinite(sample.usage[field]) || sample.usage[field] < 0) throw new Error(`Missing usage field ${field}`);
    for (const field of ["requests", "retries", "elapsed_ms", "plan_review_items"] as const) if (!Number.isFinite(sample[field]) || sample[field] < 0) throw new Error(`Missing sample field ${field}`);
  ...
  }))])) as Record<(typeof variants)[number], Record<string, { control: number; total: number; uncached_input: number; cached_input: number; output: number; requests: number; retries: number; elapsed_ms: number }>>;
  const sum = (variant: "baseline" | "candidate", field: "control" | "total") => Object.values(medians[variant]).reduce((total, item) => total + item[field], 0);
...
export function renderLiveMarkdown(config: LiveBenchmarkConfig, report: ReturnType<typeof evaluateLiveSamples>): string {
...
export async function verifyBenchmarkPackage(root: string, expected: { name: string; version: string; expected_root_hash?: string }): Promise<{ root_hash: string }> {
  const manifest = JSON.parse(await readFile(path.join(root, "package.json"), "utf8")) as { name?: string; version?: string };
