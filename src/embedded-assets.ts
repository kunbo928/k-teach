...
interface EmbeddedAssetsDocument {
  schema_version: 1;
  lesson_id: string;
...
export interface EmbeddedAssetResolution {
...
export async function resolveEmbeddedAssets(
  ...
  lesson: LessonBundle,
  markdown: string,
): Promise<EmbeddedAssetResolution> {
  ...
  const document = value as EmbeddedAssetsDocument;
  if (
