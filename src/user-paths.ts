...
function userHome(options: UserPathOptions): string {
  const environment = options.environment ?? process.env;
  return options.home ?? environment.HOME ?? environment.USERPROFILE ?? homedir() ?? options.cwd ?? process.cwd();
}

export function userConfigDir(options: UserPathOptions = {}): string {
  const environment = options.environment ?? process.env;
...
export function userCacheDir(options: UserPathOptions = {}): string {
  const environment = options.environment ?? process.env;
