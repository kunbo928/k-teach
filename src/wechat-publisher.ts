...
type ConfirmPublish = (summary: PublishSummary) => Promise<boolean>;
...
function attemptsDir(cwd: string): string {
  return path.join(cwd, ".k-teach", "publication-attempts");
}

function attemptPath(cwd: string, id: string): string {
...
async function loadAttempt(cwd: string, id: string): Promise<StoredAttempt> {
  try {
    const value = JSON.parse(await readFile(attemptPath(cwd, id), "utf8")) as StoredAttempt;
...
function cachePath(options: WechatPublisherOptions): string {
  const base =
    options.cacheDir ??
...
async function cachedToken(options: WechatPublisherOptions): Promise<string | undefined> {
  try {
    const value = JSON.parse(await readFile(cachePath(options), "utf8")) as {
      access_token?: string;
      expires_at?: number;
    };
    if (
...
function errorForWechat(code: number, message: string): KTeachError {
  let errorCode: ErrorCode = "remote-rejected";
...
async function getAccessToken(options: WechatPublisherOptions): Promise<string> {
...
function unknownError(operation: string): KTeachError {
  return new KTeachError(
    "remote-unknown",
...
export async function createWechatDraft(
  ...
  const manifest = manifestValue as unknown as WechatArtifact;
...
async function attemptsForArtifact(cwd: string, artifactId: string, accountAlias: string): Promise<StoredAttempt[]> {
  const files = await readdir(attemptsDir(cwd)).catch(() => []);
  const attempts: StoredAttempt[] = [];
  for (const file of files.filter((name) => name.endsWith(".json")).sort()) {
    try {
      const value = JSON.parse(await readFile(path.join(attemptsDir(cwd), file), "utf8")) as StoredAttempt;
...
export async function createOrReuseAuthorizedWechatDraft(artifactDir: string, options: WechatPublisherOptions): Promise<PublicationAttempt> {
  const manifestValue = JSON.parse(await readFile(path.join(artifactDir, "manifest.json"), "utf8")) as Record<string, unknown>;
...
export async function queryWechatStatus(
  ...
  try {
    ...
    const detail = result.article_detail as
      | { item?: Array<{ article_url?: string }> }
      | undefined;
