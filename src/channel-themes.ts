import type { ChannelThemeId, PublicationBrief } from "./domain.ts";
...
export type ChannelThemePattern = "rule" | "columns" | "soft-bar";
...
export const CHANNEL_THEMES: readonly ChannelTheme[] = [
  {
...
export function isChannelThemeId(value: unknown): value is ChannelThemeId {
  return (
    typeof value === "string" &&
    (CHANNEL_THEME_IDS as readonly string[]).includes(value)
  );
}

export function resolveChannelTheme(value: unknown): ChannelTheme {
...
function clipboardRuntime(articleExpression: string): string {
...
export function wrapChannelThemeProposals(
  title: string,
  articles: Record<ChannelThemeId, string>,
  selected: ChannelThemeId,
): string {
